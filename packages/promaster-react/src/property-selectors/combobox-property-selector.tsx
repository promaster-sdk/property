...
export function ComboboxPropertySelector({
	...
	const value = propertyValueSet.getInteger(propertyName);

	if (!valueItems)
		valueItems = [];
	const selectedValueItemOrUndefined = valueItems.find((item) => (item.value && item.value.getInteger()) === value);
...
function _doOnChange(newValue: any, onValueChange: (newValue: PropertyValue | null) => void) {
	if (newValue === "") {
		onValueChange(null);
	}
	else {
