  ...
  switch (component.type) {
    ...
    case "text":
      ...
      if (component.strokeThickness > 0 && component.strokeColor) {
        cs.push(<text key={key + "shadow"} x={component.position.x}
          y={component.position.y} dy={dy.toString() + "em"} style={shadowStyle} transform={transform}>{tSpans}</text>);
      }
    ...
    case "polyline":
      ...
      return [<polyline key={key} points={linePoints} stroke={colorToRgb(component.strokeColor)}
        strokeWidth={component.strokeThickness} fill="none" />];
    case "polygon":
