...
export function getLinearTicks(desiredTicks: number, min: number, max: number): Array<number> {
  const alternatives = linearPowers.reduce((currAlts: Array<Array<number>>, power: number) => {
  ...
  const bestLines = alternatives.reduce((prev: Array<number>, alt: Array<number>) =>
...
export function transformPoint(point: AbstractImage.Point, xMin: number, xMax: number, yMin: number, yMax: number,
                               xAxis: Axis | undefined, yAxis: Axis | undefined): AbstractImage.Point {
