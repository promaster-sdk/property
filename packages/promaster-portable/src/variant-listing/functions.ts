...
export function buildAllPropertyValueSetsExtended(explicitPropertyValueSet: PropertyValueSet.PropertyValueSet, variableProperties: ProductProperty[], allProperties: ProductProperty[], limit: number): ExtendedVariants {
  ...
  const newVariableProperties = variableProperties
    .filter((property) => { // remove all no discrete properties
      if (Units.getStringFromQuantityType(property.quantity).toLocaleLowerCase() !== "discrete") {
        blackListedProperties.push(property);
        if (property.validation_filter.text !== "") { blacklistedPropertyFilters.push(property.validation_filter); }
        if (property.visibility_filter.text !== "") { blacklistedPropertyFilters.push(property.visibility_filter); }
        return false;
      } else {
        return true;
      }
    ...
    .map((property) => { // remove all propertyValues that are blacklisted
      const newPV = property.value.filter((value) => blacklistedPropertyFilters.reduce((acc, bpf) => acc && !(PropertyFilter.isValid({[property.name]: value.value}, bpf)), true));
      return Object.assign({}, property, {value: newPV});
    });

  // Error check.
  if (newVariableProperties.find(property => Units.getStringFromQuantityType(property.quantity).toLocaleLowerCase() !== "discrete")) {
      ...
      .map((partialPropertyValueSet) => {
        return !property.value
          ...
          .map(propertyValueItem => {
            ...
            const propertyValueSet = R.mergeWith(R.merge, partialPropertyValueSet, {[property.name]:  { type: "integer", value: propertyValueItem.value.value as number }});
  ...
  propertyValueSets = propertyValueSets
    .filter(propertyValueSet => allProperties                          // PropertyValueSets needs to have...
      .filter((property) => !!property.value && !!property.value.length) // ...their every (discrete) propertyValue...
      .every((property) => {                                             // ...set to a valid value.
  ...
  const extendedVariants: VariantUrlList[] = propertyValueSets.map((variant) => ({
    variants: variant,
    url: Object.keys(variant).map((property) => `${property}=${variant[property].value}`).join("&")}
  ), {});
