...
import { PropertyValueSet, PropertyValue, Amount, PropertyFilter, Units } from "@promaster/promaster-primitives";
...
export function buildOrderingCodes(codeRows: ReadonlyArray<Api.ProductCode>,
  propertyValueSet: PropertyValueSet.PropertyValueSet): ReadonlyArray<BuiltOrderingCode> {
  const matchingCodeParts = codeRows.filter((c) => PropertyFilter.isValid(propertyValueSet, c.property_filter));
