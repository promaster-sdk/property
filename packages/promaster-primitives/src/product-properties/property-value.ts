...
export function create(type: PropertyType, value: Amount.Amount<any> | string | number): PropertyValue {
...
export function equals(other: PropertyValue, value: PropertyValue): boolean {
...
function _fromSerializedStringOrUndefinedIfInvalidString(encodedValue: string): PropertyValue | undefined {
    ...
    if (encodedValue.startsWith("\"") && encodedValue.endsWith("\"")) {
        ...
        if (unitString.toLowerCase() == "integer") {
            let integerValue: number = parseInt(split2[0]);
        ...
        let integerValue: number = parseInt(encodedValue);
        if (integerValue == null)
            return undefined;
        deserializedValue = fromInteger(integerValue);
    }
