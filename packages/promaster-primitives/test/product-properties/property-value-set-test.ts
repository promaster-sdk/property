...
describe('property_value_set_test', () => {
	...
	/*
	 ...
	 it('propertyvalueset_hash_should_be_same_for_same_value', () => {
	 let pvs1 = PropertyValueSet.parse("a=2;b=4:Celsius");
	 let pvs2 = PropertyValueSet.parse("a=2;b=4:Celsius");
	 let hash1 = pvs1.hashCode;
	 let hash2 = pvs2.hashCode;
	 assert.equal(hash1 == hash2, true);
	 });

	 it('propertyvalueset_equals1', () => {
	 let pvs1 = PropertyValueSet.parse("a=2;b=4:Celsius");
	 let pvs2 = PropertyValueSet.parse("a=2;b=4:Celsius");
	 assert.equal(pvs1 == pvs2, true);
	 });

	 it('propertyvalueset_equals2', () => {
	 let pvs1 = PropertyValueSet.parse("a=2;b=4:Celsius");
	 let pvs2 = PropertyValueSet.parse("a=2;b=4.5:Celsius");
	 assert.equal(pvs1 == pvs2, false);
	 });
	 */
	...
	describe('getValue', () => {
		...
		it('should get text value', () => {
			...
			assert.deepEqual(PropertyValueSet.getValue("message", pvs), PropertyValue.fromText("MyMessage"));
		});

		it('should get integer value', () => {
			let pvs = PropertyValueSet.fromString('message=12');
			assert.deepEqual(PropertyValueSet.getValue("message", pvs), PropertyValue.fromInteger(12));
		});

		it('should get amount value', () => {
			let pvs = PropertyValueSet.fromString('message=12:Celsius');
			assert.deepEqual(PropertyValueSet.getValue("message", pvs), PropertyValue.fromAmount(Amount.create(12, Units.Celsius, 0)));
		});
	...
	describe('set', () => {
		...
		it('should set the specified property', ()=> {
			...
			assert.deepEqual(PropertyValueSet.get("a", pvs2), PropertyValue.fromInteger(1));
			assert.deepEqual(PropertyValueSet.get("b", pvs2), PropertyValue.fromInteger(5));
			assert.deepEqual(PropertyValueSet.get("c", pvs2), PropertyValue.fromInteger(3));
		});
	...
	describe('setAmount', () => {
		...
		it('should set the specified property', ()=> {
			let pvs = PropertyValueSet.fromString("a=1;b=2;c=3");
			let pvs2 = PropertyValueSet.setAmount("b", Amount.create(12, Units.Celsius), pvs);
  ...
  describe('toString', () => {
	...
	// 		PropertyValueSet.fromProperty("message", "PlainString")
	// 	});
	// 	assert.equal(pvs1.equals(pvs2), true);
	// });
